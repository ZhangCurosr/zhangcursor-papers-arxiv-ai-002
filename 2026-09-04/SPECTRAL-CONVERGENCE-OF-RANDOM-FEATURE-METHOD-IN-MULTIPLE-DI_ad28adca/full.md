...
(2) Suppose $k \in L ^ { 1 } (  { \mathbb { R } } ^ { d } )$ and $\widehat { k } \simeq e ^ { - 2 \kappa | \cdot | ^ { s } }$ with $\kappa , s > 0$ . Let $t \ge 0 , 1 \le p \le \infty$ , and $0 < \theta \leq 1$ . Fix $\ell \geq t$ i $f p = 2$ , and fix any $\ell > t + d / 2$ otherwise. Then every $f \in \mathcal { H } _ { k } ^ { \theta } ( \Omega )$ is smooth and
